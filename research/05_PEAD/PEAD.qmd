...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
    labs(
        x = "Days After Earnings Release",
        y = "Earnings Surprise ($) Decile",
        title = "Average Log Returns after Earnings Release",
        subtitle = "Markets are Inefficient in the Short-Term",
        caption = "Illustration made by Max Sands w/ data from Alpha Query"
    ) +
    theme_max +
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_pct$data %>% 
    ggplot(aes(name, eps_surprise_pct, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
    labs(
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal_2015$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal_2016$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal_2017$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal_2018$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal_2019$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal_2020$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal_2021$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
...
#| fig-height: 6.75
#| fig-align: center
plots_list$heatmap_nominal_2022$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
...
#| fig-width: 8
#| fig-height: 8
plots_list$heatmap_nominal_grouped_by_market_cap$data %>% 
    ggplot(aes(name, eps_surprise_nominal, fill = avg_ret)) +
    geom_tile() +
    geom_label(aes(label = value_text), fill = "white") +
    scale_fill_gradient2(midpoint = 0, high = scales::muted("green")) +
    facet_wrap(~market_cap) +
    labs(
        x = "Days After Earnings Release",
        y = "Earnings Surprise ($) Decile",
        title = "Average Log Returns after Earnings Release",
...
#| fig-width: 8
#| fig-height: 6.5
plots_list$monte_carlo_decile_5day$data %>% 
    ggplot(aes(day, median_return, color = as.factor(return_quantile))) +
    geom_smooth(se = F) +
    scale_color_brewer(type = "div", palette = 1) +
    labs(
        x = "Days",
        y = "Wealth Index ($1)",
        title = "Monte Carlo Simulation: PEAD Long Strategy",
...
#| fig-width: 8
#| fig-height: 6.5
plots_list$monte_carlo_deciles_1day$data %>% 
        ggplot(aes(day, median_return, color = as.factor(return_quantile))) +
    geom_smooth(se = F) +
    scale_color_brewer(type = "div", palette = 1) +
    labs(
...
g3 <- data_prep %>%
    ...
    filter(!(dow %in% c("Sat", "Sun"))) %>%
    ggplot(aes(n)) +
    geom_density(fill = palette_light()[1], alpha = .8) +
    theme_tq(base_size = 14) +
    labs(
        x = "",
        y = ""
...
(g4 / g1) + plot_annotation(title = "Distribution of Earnings Releases per Day", theme = theme_tq(base_size = 13) + theme(plot.title = element_text(hjust = .5)))
```
