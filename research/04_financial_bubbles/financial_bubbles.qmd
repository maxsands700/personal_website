...
class_3_company_returns <- read_rds(here("raw_data", "Financial Bubbles", "class_3_ann_ret.rds"))

class_3_returns <- class_3_company_returns %>% 
    group_by(start_date, classification_3) %>% 
    summarize(avg_ann_ret = mean(ann_ret, na.rm = T)) %>% 
    ungroup() %>% 
    mutate(end_date = start_date + years(1)) %>% 
    left_join(
        r3000_raw %>% 
            distinct(classification_3, .keep_all = T) %>% 
            select(classification_1, classification_3) %>% 
            arrange(classification_1)
    )
```
...
In his book [*Principles for Navigating Big Debt Crises*](https://www.bridgewater.com/big-debt-crises/principles-for-navigating-big-debt-crises-by-ray-dalio.pdf), Ray Dalio posits that markets are cyclical and that financial bubbles form for the same underlying reasons - healthy growth turns to unhealthy growth as entities over-lever which eventually yields the bust of a financial bubble; he outlines the following rough framework for identifying financial bubbles with the associated message:
...
4.  Does monetary tightening risk popping the bubble?*\**
...
for (i in seq_along(classification_1_vec)){
    ...
    geom_rect(
        data = class_3_returns %>% 
            filter(classification_1 == classification_1_vec[i]),
        mapping = aes(xmin = start_date, xmax = end_date, ymin = -Inf, ymax = Inf, fill = avg_ann_ret, y = NULL, x = NULL, color = NULL),
        alpha = .5,
        show.legend = F
    ) +
    scale_fill_gradient2(low = "red", mid = "white", high = "green", na.value = "grey50", midpoint = 0) +
    geom_line() +
    geom_hline(yintercept = 0, color = "black", lty = 2) +
    ...
    labs(x="", y = "", title = "Price Metric", color = "",
         caption = "Color Scale represent average yearly nominal return of the companies in each subsector with an 80th+ percentile of Market Cap - midpoint of the color scale is 0%.") +
    scale_y_continuous(labels = scales::label_number()) +
...
for (i in seq_along(classification_1_vec)){
    ...
    scale_fill_gradient2(low = "red", mid = "white", high = "green", na.value = "grey50", midpoint = 0) +
            ...
            scale_color_tq() +
            labs(
                x="", y="", title = "IPO Metric", caption = "Color Scale represent average yearly nominal return of the companies in each subsector with an 80th+ percentile of Market Cap - midpoint of the color scale is 0%."
            ) +
            scale_x_date(labels = scales::label_date(format = "'%y"), date_breaks = "5 years") +
            scale_y_continuous(labels = scales::label_number()) +
        theme(plot.title = element_text(hjust = .5), plot.caption = element_text(size = 10, face = "italic"))
...
4.  The effects of bubbles in a particular segment tend to affect other segments as well, even if they appear unrelated, with varying degrees of magnitude.
    i.  In particular, we note that the *Financials* sector is often affected by bubbles in other segments. This is fairly intuitive, as the *Financials* sector implicitly interacts with all other sectors, and is inherently levered.
...
for (i in seq_along(classification_1_vec_fbm)){
    ...
    scale_color_tq() +
    facet_wrap(~classification_3, scales = "free", ncol = 2) +
    labs(x="", y = "", title = "Grand Financial Bubble Metric", color = "", caption = "Color Scale represent average yearly nominal return of the companies in each subsector with an 80th+ percentile of Market Cap - midpoint of the color scale is 0%.") +
    scale_y_continuous(labels = scales::label_number()) +
    scale_x_date(labels = scales::label_date(format = "'%y"), date_breaks = "5 years") +
    theme(legend.position = "top", plot.title = element_text(hjust = .5),
          plot.caption = element_text(size = 10, face = "italic"))
